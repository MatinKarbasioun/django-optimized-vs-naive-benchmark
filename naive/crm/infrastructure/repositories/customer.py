...
class CustomerRepository(ICustomerRepository):
    ...
    def remove(self, customer_id: int) -> bool:
        try:
            AppUserModel.objects.get(id=customer_id).delete()
            return True
    ...
    def is_exist(self, customer_id: str) -> bool:
        return AppUserModel.objects.filter(customer_id=customer_id).exists()

    def find_by_id(self, customer_id: int) -> Optional[Customer]:
        try:
            model = AppUserModel.objects.get(id=customer_id)
            return model @ ModelToCustomer()
